 ...
 *
 * Checks accounts table first for legacy connected GitHub accounts,
 * then users table for GitHub primary accounts.
 */
export async function getOAuthToken(
  ...
  try {
    ...
    const user = await db
      ...
      .from(users)
      .where(and(eq(users.id, userId), eq(users.provider, provider)))
      .limit(1)
